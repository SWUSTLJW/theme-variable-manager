...
`theme-variable-manager` 是一个用于管理多主题变量的工具，帮助开发者动态修改和维护主题的 `.less` 文件。
...
1. 通过 npm 安装该工具：
   npm install theme-variable-manager


## 使用指南
1. 生成配置文件
安装工具后，你可以使用以下命令生成默认的 themeConfig.json 配置文件：
npx theme-variable-manager generate-config
...
npx theme-variable-manager start-tool
该命令将启动一个本地服务，你可以通过浏览器访问并编辑主题变量。

3. 自定义配置文件路径
你可以通过命令行参数指定自定义的配置文件路径：
npx theme-variable-manager start-tool --config ./path/to/your/themeConfig.json
项目结构
|-- themes/                // 主题文件存放路径
    |-- theme1.less        // 主题1文件
    |-- theme2.less        // 主题2文件
...
npx theme-variable-manager start-tool --config ./your/config/path/themeConfig.json
